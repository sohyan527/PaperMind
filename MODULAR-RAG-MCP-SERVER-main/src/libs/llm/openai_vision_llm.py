...
class OpenAIVisionLLM(BaseVisionLLM):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.timeout = float(kwargs.get("timeout", self.DEFAULT_TIMEOUT))
        self.max_retries = int(kwargs.get("max_retries", self.DEFAULT_MAX_RETRIES))
    ...
    def _call_api(
            ...
            for attempt in range(1, self.max_retries + 1):
                try:
                    ...
                    if response.status_code not in {429, 500, 502, 503, 504}:
                        raise OpenAIVisionLLMError(
                            f"[OpenAI Vision] API error (HTTP {response.status_code}): "
                            f"{error_detail}"
                        )
                    ...
                    last_error = OpenAIVisionLLMError(
                        f"[OpenAI Vision] Request timed out after {self.timeout:g} seconds"
                    )
        ...
        raise OpenAIVisionLLMError("[OpenAI Vision] API call failed without response")
