...
DEFAULT_MAX_WORKERS = 1  # Vision endpoints can disconnect under concurrent base64 uploads
DEFAULT_MAX_IMAGES_PER_DOCUMENT = 80
MIN_CAPTION_IMAGE_DIMENSION = 80
MIN_CAPTION_IMAGE_AREA = 6400
...
class ImageCaptioner(BaseTransform):
    ...
    def transform(
        ...
        caption_candidates: Dict[str, Tuple[str, dict]] = {}  # img_id -> (img_path, metadata)
        for chunk in chunks:
            referenced_ids = self._find_referenced_image_ids(chunk.text)
            for img_id in referenced_ids:
                if img_id not in caption_candidates:
                    img_meta = image_lookup.get(img_id)
                    if img_meta and img_meta.get("path"):
                        caption_candidates[img_id] = (img_meta.get("path"), img_meta)

        images_to_caption = self._select_caption_candidates(caption_candidates)
    ...
    def _select_caption_candidates(
        ...
        """Filter and cap image caption candidates for large documents."""
        total = len(candidates)
        if total <= DEFAULT_MAX_IMAGES_PER_DOCUMENT:
            return {img_id: path for img_id, (path, _) in candidates.items()}

        eligible = []
        skipped_small = 0
        for img_id, (path, img_meta) in candidates.items():
            if self._is_caption_candidate(img_meta):
                eligible.append((img_id, path, self._image_area(img_meta)))
            else:
                skipped_small += 1

        if not eligible:
            eligible = [
                (img_id, path, self._image_area(img_meta))
                for img_id, (path, img_meta) in candidates.items()
            ]

        eligible.sort(key=lambda item: item[2], reverse=True)
        selected = eligible[:DEFAULT_MAX_IMAGES_PER_DOCUMENT]
        skipped_by_cap = max(len(eligible) - len(selected), 0)

        logger.info(
            "Caption candidate filtering: total=%d, selected=%d, "
            "skipped_small=%d, skipped_by_cap=%d",
            total,
            len(selected),
            skipped_small,
            skipped_by_cap,
        )
        return {img_id: path for img_id, path, _ in selected}

    def _is_caption_candidate(self, img_meta: dict) -> bool:
        position = img_meta.get("position") or {}
        width = int(position.get("width") or 0)
        height = int(position.get("height") or 0)
        if width <= 0 or height <= 0:
            return True
        return (
            width >= MIN_CAPTION_IMAGE_DIMENSION
            and height >= MIN_CAPTION_IMAGE_DIMENSION
            and width * height >= MIN_CAPTION_IMAGE_AREA
        )

    def _image_area(self, img_meta: dict) -> int:
        position = img_meta.get("position") or {}
        width = int(position.get("width") or 0)
        height = int(position.get("height") or 0)
        return width * height
